...
int TablePage::UpdateTuple(const Row &new_row, Row *old_row, Schema *schema,
  ...
  // 1.If there is not enough space to update, we need to update via delete followed by an insert (not enough space).
  // 2.It Means this Page Can not Allocate the current Size of the Updated row.
  // 3.So we need to Delete the Tuple First and Allocate this tuple to other Page.
  ...
  // Change free_space_pointer to deleted old tuple position
  SetFreeSpacePointer(free_space_pointer + tuple_size - serialized_size);

  //Serialize the New Row to the buffer
