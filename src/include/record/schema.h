...
public:
  ...
  /**
   * Shallow copy schema, only used in index, do not actually allocate space for columns, thus the pointer stored
   * in the copy destinition schema is the same as the pointer stored in the copied schema (source)
   *
  ...
  static Schema *ShallowCopySchema(const Schema *table_schema, const std::vector<uint32_t> &attrs, MemHeap *heap) {
    ...
    cols.reserve(attrs.size()); // reserve increase the capacity of the vector (reserve storage for vector)
  ...
  /**
   * Deep copy schema: do the actual object allocate and copy for the columns.
   */
...
private:
  static constexpr uint32_t SCHEMA_MAGIC_NUM = 200715; // used to test schema serialization. (schema's ID)
  std::vector<Column *> columns_;   /** don't need to delete pointer to column */
};

// using a = b syntax -> specify the alias of the namespace
using IndexSchema = Schema;
using TableSchema = Schema; 
// here TableSchema and IndexSchema are both Schema's alias, when specifying them, we are actually specifying Schema namespace
