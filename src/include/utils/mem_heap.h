...
class MemHeap {
// this is an extentable base class -> this class is used to manage the storage of memory by using the heap file organization
public:
...
public:
  ...
  void Free(void *ptr) {
    ...
    if (iter != allocated_.end()) { // The corresponding element is found.
      allocated_.erase(iter);
    }
  }

private:
  std::unordered_set<void *> allocated_; // use hash table to store the element, allows fast access
};
